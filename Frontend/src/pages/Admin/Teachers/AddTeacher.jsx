...
const AddTeacher = () => {
  ...
  const handleChange = async (e) => {
    ...
    if (errors[name]) {
      try {
        ...
        await teacherSchema.validateAt(name, newFormData, { context: { isAddMode: true } });
        setErrors((prev) => ({ ...prev, [name]: '' }));
      } catch (error) {
  ...
  const handleBlur = async (e) => {
    ...
    try {
      await teacherSchema.validateAt(name, formData, { context: { isAddMode: true } });
      setErrors((prev) => ({ ...prev, [name]: '' }));
    } catch (error) {
      if (error.name === 'ValidationError') {
        setErrors((prev) => ({ ...prev, [name]: error.message }));
        ...
        <div className="flex items-center justify-between">
          ...
          <div>
            ...
            <p className="text-slate-400 mt-1">
                Create a new teacher profile and professional information.</p>
